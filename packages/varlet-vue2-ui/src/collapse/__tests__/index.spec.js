...
test('test collapse and collapseItem plugin', () => {
  ...
  expect(Vue.component(Collapse.name)).toBeTruthy()
  expect(Vue.component(CollapseItem.name)).toBeTruthy()
})
...
test('test collapse modelValue and onChange', async () => {
  ...
  const secondCollapseItemHeader = collapseItemList.at(1).find('.var-collapse-item-header')
  ...
  expect(collapseItemList.at(0).classes()).toContain('var-collapse-item__active')
...
test('test collapseItem disabled', async () => {
  ...
  await collapseItemList.at(0).find('.var-collapse-item-header').trigger('click')
